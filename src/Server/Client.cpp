...
/*   Updated: 2025/06/12 16:41:56 by jdhallen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Client::Client()
	: client_fd(-1), nickname(""), username(""), authenticated(false),
		is_in_channel(false), channel(""){
			
}

Client::Client(int _client_fd)
	: client_fd(_client_fd), nickname(""), username(""), authenticated(false),
		is_in_channel(false), channel(""){
}
...
Client::~Client(){
	if (client_fd != -1)
		close(client_fd);
	std::cout << GREY << "Client leaved closed" << RESET << std::endl;
}
